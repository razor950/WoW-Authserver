...
exports.logon_challenge_response = function(B, s, unk)
{
    ...
    buf.writeInt8(0, pos, true); pos += 1;
    buf.writeInt16LE(0, pos, true); pos += 2;
    BNT.toBuffer(B, 32).reverse().copy(buf, pos); pos += 32;
    buf.writeInt8(1, pos, true); pos += 1;
    BNT.toBuffer(g, 1).copy(buf, pos); pos += 1;
    buf.writeInt8(32, pos, true); pos += 1;
    BNT.toBuffer(N, 32).reverse().copy(buf, pos); pos += 32;
    BNT.toBuffer(s, 32).reverse().copy(buf, pos); pos += 32;
    BNT.toBuffer(unk, 16).copy(buf, pos); pos += 16;
...
exports.generate_M = function(username, s, A, B, K)
{
    ...
    sha.digest().reverse().copy(Nh);

    sha = crypto.createHash('sha1');
    sha.update(BNT.toBuffer(g));
    var gh = new Buffer(20);
    sha.digest().reverse().copy(gh);
...
exports.logon_proof_response = function(A, M, K)
{
    ...
    res.writeInt8(1, pos, true); pos += 1;
    res.writeInt8(0, pos, true); pos += 1;
    sha.digest().copy(res, pos); pos += 20;
    res.writeUInt32LE(0x800000, pos, true); pos += 4;
    res.writeUInt32LE(0, pos, true); pos += 4;
