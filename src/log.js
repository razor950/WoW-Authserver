...
exports.Log = function(err, header)
{
    ...
    var msg = date + ' ' + lvlName + ' [' + header.join('@') + '] (' + err.error + ') ' + err.message;
    var msg_c = '';
    if (LOG_COLORS)
    {
        for (var i = 0; i < header.length; ++i)
            header[i] = header[i].cyan;
        
        msg_c = date.cyan + ' ' + lvlName['LOG_' + lvlName] + ' [' + header.join('@') + '] (' + err.error.yellow + ') ' + err.message;
        hc = header.join('@');
    }
    ...
    for (var i = 0; i < LOG_TARGETS.length; ++i)
    {
        ...
        if (t.levels & (1 << err.logLevel))
            t.target(t.colors ? msg_c : msg);
    }
};
